	...
	var d="2.7.1",
		...
		n.generatedcontent=function(){
			...
			return t(["#",
				...
				':after{content:"',
				l,
				'";visibility:hidden;font:3px/1 a}'].join(""),
				function(b){
					a=b.offsetHeight>=3
				}
			),
		...
		return e.addTest=
			...
			g.className=g.className.replace(/(^|\s)no-js(\s|$)/,"$1$2") +
				(f?" supports-js supports-"+q.join(" supports-"):""),
			e
}
...
function(a,b){
	...
	function w(a){
		...
		while(d--)
			b=c[d],
			e.test(b.nodeName)&&f.push(b.applyElement(x(b)));
		return f
	}
	function x(a){
		var b,
